...
if ((isset($_POST['get_favorites'])) && ($_POST['get_favorites'] == 1)) {
    ...
    */
    ...
    if (!empty($user_id)) {
        ...
        $db->sql($sql);
        $total = $db->getResult();
        $sql = "select f.id,f.user_id,f.product_id,p.row_order,p.name,p.slug,p.category_id,(SELECT name from category c where c.id=p.category_id) as category_name,p.subcategory_id,p.indicator,p.manufacturer,p.made_in,p.return_status,p.cancelable_status,p.till_status,p.image,p.other_images,p.size_chart,p.description,p.shipping_delivery,p.status,p.date_added,p.ratings,p.number_of_ratings,pr.review from favorites f LEFT JOIN products p ON f.product_id=p.id left join product_reviews pr on p.id = pr.product_id where f.user_id=" . $user_id . " ORDER BY f.date_created DESC LIMIT $offset,$limit";
        $db->sql($sql);
        $res = $db->getResult();
