...
if (isset($_POST['update_featured_order']) && $_POST['update_featured_order'] == 1) {
    ...
    if ($permissions['featured']['create'] == 1) {
        ...
        for ($i = 0; $i < count($id_ary); $i++) {
            $sql = "UPDATE `sections` SET row_order='" . $i . "' WHERE id=" . $id_ary[$i];
            $db->sql($sql);
            $res = $db->getResult();
        }
        echo "<p class='alert alert-success'>Sections order updated!</p>";
        return false;
    } else {
        echo "<p class='alert alert-danger'>You have no permission to update Sections order</p>";
        return false;
    }
}
?>
...
<head>
    <title>Featured Sections | <?= $settings['app_name'] ?> - Dashboard</title>
...
<div class="content-wrapper">
    <div class="container">
        <h2>Featured Sections Order</h2>
        <hr>
        <div class='row'>
            <div class="col-md-6 col-sm-12 col-xs-12 refresh">
                <form id="category_form" method="POST" action="sections-order.php" data-parsley-validate class="form-horizontal form-label-left">
                    <input type="hidden" id="update_featured_order" name="update_featured_order" required value='1' />
                    <div class="form-group" style="overflow: scroll;height: 269px;overflow-x: hidden;">
                        ...
                        <ol id="sortable-row">
                            <?php foreach ($res as $sections) { ?>
                                <li id=<?php echo $sections["id"]; ?>>
                                    <?php echo "<big>" . $sections["row_order"] . ".</big> &nbsp;" . $sections["title"]; ?>
                                </li>
                            <?php } ?>
                        </ol>
                    </div>
