...
if (isset($_POST['get-all-sections'])) {
    ...
    foreach ($result as $row) {
        ...
        $sql = 'SELECT p.*,pr.review,(SELECT name FROM category c WHERE c.id=p.category_id) as category_name 
        FROM `products` p left join product_reviews pr on p.id = pr.product_id  WHERE p.status = 1 AND p.id IN (' . $product_ids . ')';
        $db->sql($sql);
        ...
        foreach ($result1 as $row) {
            ...
            $row['review'] = (!empty($row['review'])) ? DOMAIN_URL . $row['review'] : "";
