...
if ((isset($_POST['delete_product_review'])) && ($_POST['delete_product_review'] == 1)) {
    ...
    */
    ...
    if ($count > 0) {
        // echo "hello";
        $sql_query = "DELETE FROM `product_reviews` WHERE product_id = $product_id AND user_id = $user_id";
        $db->sql($sql_query);
        $res = $db->getResult();
    ...
    WHERE p.id = $product_id ";
        ...
        $sql1 = "UPDATE `products` p  INNER JOIN ( SELECT product_id, COUNT(id) as total_ratings, AVG(rate) as average FROM product_reviews WHERE product_id = $product_id ) pr ON p.id = pr.product_id p.ratings = 0, p.number_of_ratings = 0
        WHERE p.id = $product_id ";
    }
    // $res = $db->getResult();
    if ($db->sql($sql1)) {
